...
public sealed class GeneratedWhereGenerator : IIncrementalGenerator
{
    ...
    {
        var shapes = new ShapeRegistry<PredicateShape>(static shape => shape.Key);
        ...
        foreach (InvocationCandidate invocationCandidate in invocations)
        {
            ...
            PredicateShape shape = shapes.GetOrAdd(
                candidate,
        ...
        foreach (PredicateShape shape in shapes.Ordered())
        {
    ...
    private static string RenderInterceptor(WhereInterceptionSite site)
    {
        ...
        StringBuilder source = GeneratorSupport.CreateSource(24 * 1024, InterceptorHeader);
    ...
    {
        string hash = GeneratorSupport.StableName(shape.Key);
        StringBuilder source = GeneratorSupport.CreateSource(32 * 1024, Header);
        RenderPredicateDelegate(source, shape, hash);
        foreach (TerminalShape terminal in shape.Terminals.Ordered())
        {
    ...
    private sealed class PredicateShape
    {
            ...
            string[]? components)
        {
            ...
            var slots = isFunctor
                ? Components.Select((component, index) => new ComponentSlot(
                    index,
                    component,
                    SelectorKind.Inferred,
                    GeneratorSupport.AccessKindFrom(index < Pattern.Length ? Pattern[index] : 'I'))).ToImmutableArray()
                : ImmutableArray<ComponentSlot>.Empty;

            Api = new ApiShape(
                OperationKind.Where,
                TargetKind.World,
                QueryMode.Required,
                new SelectorSpec(SelectorKind.Inferred, slots),
                new ContextSpec(hasContext ? ContextModeKind.Value : ContextModeKind.None, contextType),
                new CallbackSpec(
        ...
        internal ShapeRegistry<TerminalShape> Terminals { get; } = new(static terminal => terminal.SignatureKey);
