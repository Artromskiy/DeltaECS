...
public partial class ComparativeStructuralQueryBenchmarks
{
    ...
    private int CollectMatches() { var s = new QueryCollectState(_matches); _world.Query(in _query, QueryAccess.Read, ref s, static (ref QueryCollectState state, ref DenseChunkLeaseView lease) => { var entities = lease.Entities; for (var i = entities.Length - 1; i >= 0; i--) state.Entities[state.Count++] = entities[i]; }); return s.Count; }
