...
public partial class ComparativeStructuralListBenchmarks
{
    ...
    [IterationSetup(Target = nameof(DeltaECS_List_RemoveBatch))] public void PrepareRemove() { RestoreBase(); _world.Add(_activeChange, _entities); _state = ListState.Added; }
    [IterationCleanup] public void RestoreAfterIteration() { RestoreBase(); RestoreListFallbacks(); }
    [Benchmark(Baseline = true), BenchmarkCategory("Structural.List.CreateBatch")] public int DeltaECS_List_CreateBatch() { var c = _world.Create(_targetArchetype, _created); _state = ListState.Created; return c == Amount ? c : throw new InvalidOperationException("list create count mismatch"); }
    [Benchmark(Baseline = true), BenchmarkCategory("Structural.List.DestroyBatch")] public int DeltaECS_List_DestroyBatch() { var c = _world.Destroy(_entities); _state = ListState.Destroyed; return c == Amount ? c : throw new InvalidOperationException("list destroy count mismatch"); }
    [Benchmark(Baseline = true), BenchmarkCategory("Structural.List.AddBatch")] public int DeltaECS_List_AddBatch() { var c = _world.Add(_activeChange, _entities); _state = ListState.Added; return c == Amount ? c : throw new InvalidOperationException("list add count mismatch"); }
    [Benchmark(Baseline = true), BenchmarkCategory("Structural.List.RemoveBatch")] public int DeltaECS_List_RemoveBatch() { var c = _world.Remove(_activeChange, _entities); _state = ListState.Base; return c == Amount ? c : throw new InvalidOperationException("list remove count mismatch"); }
    private void RestoreBase() { switch (_state) { case ListState.Created: _world.Destroy(_created); break; case ListState.Destroyed: _world.Create(_base, _entities); break; case ListState.Added: _world.Remove(_activeChange, _entities); break; } _state = ListState.Base; }
...
public partial class ComparativeStructuralQueryBenchmarks
{
    ...
    [Benchmark(Baseline = true), BenchmarkCategory("Structural.Query.CreateBatch")] public int DeltaECS_Query_CreateBatch() { var c = _world.Create(_targetArchetype, _created); _state = QueryState.Created; return c == Amount ? c : throw new InvalidOperationException("query create count mismatch"); }
    [Benchmark(Baseline = true), BenchmarkCategory("Structural.Query.DestroyBatch")] public int DeltaECS_Query_DestroyBatch() { var c = _world.Destroy(in _query); _state = QueryState.Destroyed; return c == ExpectedMatches ? c : throw new InvalidOperationException("query destroy selection mismatch"); }
    [Benchmark(Baseline = true), BenchmarkCategory("Structural.Query.AddBatch")] public int DeltaECS_Query_AddBatch() { var c = _world.Add(in _query, _activeChange); _state = QueryState.Mixed; return c == ExpectedMatches ? c : throw new InvalidOperationException("query add selection mismatch"); }
    [Benchmark(Baseline = true), BenchmarkCategory("Structural.Query.RemoveBatch")] public int DeltaECS_Query_RemoveBatch() { var c = _world.Remove(in _query, _activeChange); _state = QueryState.Mixed; return c == ExpectedMatches ? c : throw new InvalidOperationException("query remove selection mismatch"); }
    ...
    private void RestoreEmpty() { if (_state == QueryState.Created) _world.Destroy(_created); else if (_state == QueryState.Destroyed) _world.Destroy(_nonMatches.AsSpan(0, Amount - ExpectedMatches)); else if (_state == QueryState.Mixed) _world.Destroy(_entities); _state = QueryState.Empty; }
...
public class ComparativeStructuralAtomicBenchmarks
{
    ...
    private void AddDelta() { _delta.Add(_deltaActive, _deltaEntity); _deltaAddedState = true; } private void RemoveDelta() { _delta.Remove(_deltaActive, _deltaEntity); _deltaAddedState = false; }
