...
public class ComparativeStructuralListBenchmarks
{
    ...
    [IterationCleanup] public void RestoreAfterIteration() => RestoreBase();
...
public class ComparativeStructuralQueryBenchmarks
{
    ...
    [IterationCleanup] public void RestoreAfterIteration() => RestoreEmpty();
    ...
    private int CollectMatches() { var s = new QueryCollectState(_matches); _world.Query(in _query, QueryAccess.Read, ref s, static (ref QueryCollectState state, ref DenseChunkLeaseView lease) => { var entities = lease.Entities; for (var i = entities.Length - 1; i >= 0; i--) if (lease.IsAllSlotsActive || lease.IsActiveSlot(i)) state.Entities[state.Count++] = entities[i]; }); return s.Count; }
...
public class ComparativeStructuralAtomicBenchmarks
{
    ...
    private Arch.Core.World _arch = null!; private Arch.Core.Entity _archEntity, _archExtra; private bool _archAlive, _archExtraAlive; private EntityStore _friflo = null!; private Friflo.Engine.ECS.Entity _frifloEntity, _frifloExtra; private bool _frifloAlive, _frifloExtraAlive; private DefaultWorld _default = null!; private DefaultEcs.Entity _defaultEntity, _defaultExtra; private bool _defaultAlive, _defaultExtraAlive; private EcsWorld _leo = null!; private int _leoEntity, _leoExtra; private bool _leoAlive, _leoExtraAlive; private EcsPool<AtomicLeoBase> _leoBase = null!; private EcsPool<AtomicLeoA0> _leoA0 = null!; private EcsPool<AtomicLeoA1> _leoA1 = null!; private EcsPool<AtomicLeoA2> _leoA2 = null!; private EcsPool<AtomicLeoA3> _leoA3 = null!;
    private bool _deltaAddedState, _archAddedState, _frifloAddedState, _defaultAddedState, _leoAddedState;
    [GlobalSetup] public void Setup() { var l = new ComponentLayoutRegistry(); _deltaBase = l.Register<StructuralBase>(new SchemaId(220_000)); var added = new[] { l.Register<StructuralA0>(new SchemaId(220_001)), l.Register<StructuralA1>(new SchemaId(220_002)), l.Register<StructuralA2>(new SchemaId(220_003)), l.Register<StructuralA3>(new SchemaId(220_004)) }; _deltaActive = added.AsSpan(0, ChangeWidth).ToArray(); _delta = new DeltaWorld(l); _deltaEntity = _delta.Create(new[] { _deltaBase }); _deltaAlive = true; _arch = Arch.Core.World.Create(); _arch.Reserve(new Arch.Core.Utils.ComponentType[] { typeof(AtomicArchBase) }, 2); _arch.Reserve(new Arch.Core.Utils.ComponentType[] { typeof(AtomicArchBase), typeof(AtomicArchA0) }, 2); _arch.Reserve(new Arch.Core.Utils.ComponentType[] { typeof(AtomicArchBase), typeof(AtomicArchA0), typeof(AtomicArchA1) }, 2); _arch.Reserve(new Arch.Core.Utils.ComponentType[] { typeof(AtomicArchBase), typeof(AtomicArchA0), typeof(AtomicArchA1), typeof(AtomicArchA2) }, 2); _arch.Reserve(new Arch.Core.Utils.ComponentType[] { typeof(AtomicArchBase), typeof(AtomicArchA0), typeof(AtomicArchA1), typeof(AtomicArchA2), typeof(AtomicArchA3) }, 2); _archEntity = _arch.Create(typeof(AtomicArchBase)); _archAlive = true; _friflo = new EntityStore(); _frifloEntity = _friflo.CreateEntity(new AtomicFrifloBase()); _frifloAlive = true; _default = new DefaultWorld(); _defaultEntity = _default.CreateEntity(); _defaultEntity.Set<AtomicDefaultBase>(); _defaultAlive = true; _leo = new EcsWorld(); _leoEntity = _leo.NewEntity(); _leoAlive = true; _leoBase = _leo.GetPool<AtomicLeoBase>(); _leoA0 = _leo.GetPool<AtomicLeoA0>(); _leoA1 = _leo.GetPool<AtomicLeoA1>(); _leoA2 = _leo.GetPool<AtomicLeoA2>(); _leoA3 = _leo.GetPool<AtomicLeoA3>(); _leoBase.Add(_leoEntity); }
    ...
    private void AddArch() { _arch.Add(_archEntity, new AtomicArchA0()); if (ChangeWidth > 1) { _arch.Add(_archEntity, new AtomicArchA1()); _arch.Add(_archEntity, new AtomicArchA2()); _arch.Add(_archEntity, new AtomicArchA3()); } _archAddedState = true; } private void RemoveArch() { _arch.Remove<AtomicArchA0>(_archEntity); if (ChangeWidth > 1) { _arch.Remove<AtomicArchA1>(_archEntity); _arch.Remove<AtomicArchA2>(_archEntity); _arch.Remove<AtomicArchA3>(_archEntity); } _archAddedState = false; }
    private void AddFriflo() { _frifloEntity.AddComponent(new AtomicFrifloA0()); if (ChangeWidth > 1) { _frifloEntity.AddComponent(new AtomicFrifloA1()); _frifloEntity.AddComponent(new AtomicFrifloA2()); _frifloEntity.AddComponent(new AtomicFrifloA3()); } _frifloAddedState = true; } private void RemoveFriflo() { _frifloEntity.RemoveComponent<AtomicFrifloA0>(); if (ChangeWidth > 1) { _frifloEntity.RemoveComponent<AtomicFrifloA1>(); _frifloEntity.RemoveComponent<AtomicFrifloA2>(); _frifloEntity.RemoveComponent<AtomicFrifloA3>(); } _frifloAddedState = false; }
