...
public class ComparativeDenseIterationBenchmarks
{
    ...
    private CursorReadBinding<UnifiedDeltaValue> _deltaValueBinding;
    ...
    public void Setup()
    {
        ...
        _deltaValueBinding = _deltaQuery.CursorBind<UnifiedDeltaValue>(_deltaValue, RowAccess.Read);
    ...
    public long DeltaECS_Dense()
    {
        ...
        _delta.QueryCursor(in _deltaQuery, ref state, static (ref DenseState current, ref DenseChunkCursor cursor) =>
        {
            var row = cursor.Resolve(current.Value);
    ...
    {
        public CursorReadBinding<UnifiedDeltaValue> Value;
        public long Sum;
    }
}
...
public class ComparativeMovement2ComponentsBenchmarks
{
    ...
    private CursorWriteBinding<MoveDeltaPosition> _deltaPositionBinding;
    private CursorReadBinding<MoveDeltaVelocity> _deltaVelocityBinding;
    private Arch.Core.World _arch = null!;
    private Arch.Core.QueryDescription _archQuery;
    private ArchComponentType[] _archTypes = null!;
    ...
    public void Setup()
    {
        ...
        _deltaPositionBinding = _deltaQuery.CursorBind<MoveDeltaPosition>(_deltaPosition, RowAccess.Write);
        _deltaVelocityBinding = _deltaQuery.CursorBind<MoveDeltaVelocity>(_deltaVelocity, RowAccess.Read);
    ...
    public double DeltaECS_Movement2Components()
    {
        ...
        _delta.QueryCursor(in _deltaQuery, ref state, static (ref Movement2State current, ref DenseChunkCursor cursor) =>
        {
            var positions = cursor.Resolve<MoveDeltaPosition>(current.Position);
            var velocities = cursor.Resolve<MoveDeltaVelocity>(current.Velocity);
    ...
    {
        public CursorWriteBinding<MoveDeltaPosition> Position;
        public CursorReadBinding<MoveDeltaVelocity> Velocity;
        public double Sum;
    }
}
...
public class ComparativeMovement4ComponentsBenchmarks
{
    ...
    private CursorWriteBinding<DistinctDelta0> _delta0Binding; private CursorWriteBinding<DistinctDelta1> _delta1Binding; private CursorWriteBinding<DistinctDelta2> _delta2Binding; private CursorReadBinding<DistinctDelta3> _delta3Binding;
    ...
    [GlobalCleanup] public void Cleanup() { _defaultQuery?.Dispose(); _default?.Dispose(); }
    [Benchmark(Baseline = true)] public int DeltaECS_Movement4Components() { var state = new Movement4State { A = _delta0Binding, B = _delta1Binding, C = _delta2Binding, D = _delta3Binding }; _delta.QueryCursor(in _deltaQuery, ref state, static (ref Movement4State current, ref DenseChunkCursor cursor) => { var a = cursor.Resolve<DistinctDelta0>(current.A); var b = cursor.Resolve<DistinctDelta1>(current.B); var c = cursor.Resolve<DistinctDelta2>(current.C); var d = cursor.Resolve<DistinctDelta3>(current.D); while (cursor.MoveNext()) { ref var rowA = ref a[cursor]; ref var rowB = ref b[cursor]; ref var rowC = ref c[cursor]; ref readonly var rowD = ref d[cursor]; var updatedA = rowA.Value + rowD.Value; var updatedB = rowB.Value + rowD.Value; rowA.Value = updatedA; rowB.Value = updatedB; rowC.Value = (updatedA + updatedB) / 2; current.Sum += rowA.Value + rowB.Value + rowC.Value + rowD.Value; } }); return state.Sum; }
    ...
    {
        public CursorWriteBinding<DistinctDelta0> A;
        public CursorWriteBinding<DistinctDelta1> B;
        public CursorWriteBinding<DistinctDelta2> C;
        public CursorReadBinding<DistinctDelta3> D;
        public int Sum;
    }
...
public class ComparativeWideArchetypeNarrowQueryBenchmarks
{
    ...
    private CursorReadBinding<WideDelta0> _delta0Binding; private CursorReadBinding<WideDelta7> _delta7Binding;
    ...
    [GlobalCleanup] public void Cleanup() { _defaultQuery?.Dispose(); _default?.Dispose(); }
    [Benchmark(Baseline = true)] public int DeltaECS_WideArchetypeNarrowQuery() { var state = new WideState { A = _delta0Binding, Z = _delta7Binding }; _delta.QueryCursor(in _deltaQuery, ref state, static (ref WideState current, ref DenseChunkCursor cursor) => { var a = cursor.Resolve(current.A); var z = cursor.Resolve(current.Z); while (cursor.MoveNext()) current.Sum += a[cursor].Value + z[cursor].Value; }); return Check(state.Sum, Amount * 9); }
    ...
    {
        public CursorReadBinding<WideDelta0> A;
        public CursorReadBinding<WideDelta7> Z;
        public int Sum;
    }
...
public class ComparativeSparseQueryBenchmarks
{
    ...
    private CursorReadBinding<SparseDeltaA> _deltaABinding; private CursorReadBinding<SparseDeltaB> _deltaBBinding;
    ...
    [Benchmark(Baseline = true), BenchmarkCategory("Iteration.SparseWorldColdQuery")] public int DeltaECS_SparseWorldColdQuery() { var d = new QueryDescription(new[] { _deltaA, _deltaB }, Array.Empty<ComponentId>(), new[] { _deltaC }, Array.Empty<TagId>(), Array.Empty<TagId>(), Array.Empty<TagId>()); var query = _delta.CreateQuery(in d); var a = query.CursorBind<SparseDeltaA>(_deltaA, RowAccess.Read); var b = query.CursorBind<SparseDeltaB>(_deltaB, RowAccess.Read); return DeltaQuery(query, a, b); }
    ...
    private int DeltaQuery(QueryHandle query, CursorReadBinding<SparseDeltaA> a, CursorReadBinding<SparseDeltaB> b) { var state = new SparseDeltaState { A = a, B = b }; _delta.QueryCursor(in query, ref state, static (ref SparseDeltaState current, ref DenseChunkCursor cursor) => { var aRow = cursor.Resolve(current.A); var bRow = cursor.Resolve(current.B); while (cursor.MoveNext()) { _ = aRow[cursor]; _ = bRow[cursor]; current.Count++; } }); return Check(state.Count); }
    ...
    {
        public CursorReadBinding<SparseDeltaA> A;
        public CursorReadBinding<SparseDeltaB> B;
        public int Count;
    }
}
