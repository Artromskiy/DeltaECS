...
The same class also exposes the independent dense archetype/chunk/slot path for an
addressable comparison:

```bash
env NuGetAudit=false RestoreIgnoreFailedSources=true \
dotnet benchmarks/DeltaECS.MicroBenchmarks/bin/Release/net8.0/DeltaECS.MicroBenchmarks.dll \
  --filter '*QueryIteratorIterationMicroBenchmarks.*IndependentIterators*' \
  --artifacts artifacts/micro/independent-dense-iterators
```

Those methods use `IterateDense()`, then independent archetype, chunk and slot
iterators. The baseline methods retain the combined
`MoveNextArchetype()`/`MoveNextChunk()`/`CurrentChunk` path.
