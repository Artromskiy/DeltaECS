...
Options:
  --adaptive        Let BenchmarkDotNet choose warmups, iterations and launches
...
[$started_at] Starting elevated comparative iteration benchmark
  ...
  mode:        $([[ "$adaptive" == true ]] && echo adaptive || echo fixed)
  warmups:     $([[ "$adaptive" == true ]] && echo BDN-default || echo "$warmup_count")
  iterations:  $([[ "$adaptive" == true ]] && echo BDN-default || echo "$iteration_count")
  launches:    $([[ "$adaptive" == true ]] && echo BDN-default || echo "$launch_count")
  filter:      $filter
