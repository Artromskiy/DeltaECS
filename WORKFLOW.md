...
must keep the same version. The current package version is `0.0.12`. From the
...
dotnet nuget push "$package_dir/DeltaECS.0.0.12.nupkg" \
  --api-key "$NUGET_API_KEY" \
  --source https://api.nuget.org/v3/index.json \
  --skip-duplicate
dotnet nuget push "$package_dir/DeltaECS.Generators.0.0.12.nupkg" \
  --api-key "$NUGET_API_KEY" \
  --source https://api.nuget.org/v3/index.json \
  --skip-duplicate
```
