...
- Baseline raw JIT (candidate worktree artifact): `<external-baseline-root>/deltaecs-v3-baseline-2/artifacts/jit-disasm/query-plan-matching-baseline-release.txt`
...
The next parallel implementation `eeec3ea` reused the existing native and plan
buffers and was a separate candidate; it is intentionally not included in this
