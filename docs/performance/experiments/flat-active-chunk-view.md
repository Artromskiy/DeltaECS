...
on chunk activation/deactivation, can remove the archetype scan from the former
two-loop traversal path and make generated write-tick reservation use an exact
O(1) active count. Generated `ForEach` and the old two-loop path consumed the
flat view; the old explicit traversal form was later removed.
