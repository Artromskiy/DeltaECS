...
| Array row-start reference candidates | Compare `array[0]`, Span, `fixed`, and runtime-specific `Unsafe.As` offset in generated `ForEach<T>` | 200K rows, 70×100 ms; Mono 6.12 and CoreCLR 10; [full evidence](netstandard21-array-reference-2026-09-17.md) | `fixed` showed no repeatable gain; offset is CoreCLR-only and slower in final pass; Mono pass was CPU-contended, so comparative result is inconclusive |
