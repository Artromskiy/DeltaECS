...
| Reuse one `GeneratedQuerySlots` ref-struct instance across chunk transitions | Candidate source experiment in `src/DeltaECS/Generator/GeneratedQuerySlots.cs`, `GeneratedRuntime.cs` and `DemandDrivenForEachGenerator.cs`; candidate JIT probe `artifacts/perf-round-20260829/candidate-reusable-slots-jit.txt` | Rejected at the language-safety gate: passing the `ref struct` through the execution receiver produced CS8350/CS8352 ref-escape errors, and the attempted BDN used a stale binary before a successful candidate build, so it is not performance evidence. No source change retained. |
| Generated closed-path routes instead of prepared access-token values | Candidate JIT `artifacts/perf-round-20260829/candidate-route-only-jit.md`; 300-ms BDN `artifacts/perf-round-20260829/candidate-route-only-300-bdn` versus `/private/tmp/deltaecs-perf-baseline3-20260829/artifacts/perf-round-20260829/baseline-route-only-300-bdn` | Rejected after the stronger recheck. The generated setup used validated `int` routes while retaining query/world/type validation, but JIT grew `888→904 B`; `blr=5` and `sbfiz=4` were unchanged. Candidate/base Movement4 deltas were `−11.41%/+2.52%/+2.14%/+3.92%/−0.15%` at `100/1K/10K/100K/1M` entities. All five `Mean ± Error` intervals overlapped (`457.9±143.1` vs `516.9±226.8 ns` at 100; `1,854.0±129.6` vs `1,808.5±171.5 ns` at 1K; `14,109.2±1,972.8` vs `13,813.4±971.1 ns` at 10K; `127,430.1±7,966.5` vs `122,617.4±6,326.2 ns` at 100K; `1,281,214.6±58,881.9` vs `1,283,107.0±74,056.7 ns` at 1M). Allocations stayed at `0 B`; the candidate was reverted. |
