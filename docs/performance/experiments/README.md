...
| --- | --- | --- |
| Propagate `--amount`/`--amounts` through an environment variable inherited by the BenchmarkDotNet child process | The first `RemainingApiOptimizationMicroBenchmarks` run reported empty-world timings although the host runner selected `100/1K/10K`; corrected runs are under `artifacts/remaining-api-{baseline-correct,candidate-final}` | Fixes a benchmark-runner bug: a host-process static field is not shared with the generated BDN process. Results produced before this correction are invalid and are not used below. |
...
| Filtered entity sequences | Reuse the query plan's O(1) archetype-membership table, cache the last archetype decision, and split filtered/unfiltered loops so the unfiltered loop has no mode branch | Baseline checkpoint `9c33856` under `<baseline-worktree>/artifacts/remaining-api-baseline-correct`; final candidate under `artifacts/remaining-api-accepted-final2`; .NET 10.0.9 Arm64, tiering/R2R disabled, `10` warmups, `10–20 × 300ms`, one launch | `FilteredSequence` changed from `407.46ns / 4.029μs / 40.203μs` to `183.72ns / 1.788μs / 17.904μs` at `100/1K/10K` entities: ratios `0.451 / 0.444 / 0.445`, all `0 B`. The same prepared-membership collector is used by filtered `Add`, `Remove`, and `Destroy`. |
| Generated entity sequences | Validate once, consume prepared component routes, use trusted cursor endpoints under one lease, and borrow the matching plan instead of copying it per entity | Baseline checkpoint `9c33856` under `<baseline-worktree>/artifacts/remaining-api-baseline-generated-sequence`; final candidate under `artifacts/remaining-api-accepted-final2`; generator/runtime tests include nonzero-slot and foreign-query cases | `GeneratedFilteredSequence` changed from `345.5ns / 3.368μs / 34.11μs` to `278.58ns / 2.748μs / 28.264μs` at `100/1K/10K`: ratios `0.806 / 0.816 / 0.829`, all `0 B`. The change also fixes the former cursor endpoint reading slot zero for every selected entity; because that bug gave the baseline artificially favorable repeated-slot locality, this throughput comparison is conservative rather than an equal-semantics claim. |
| Dense stamp observation | Resolve chunk/archetype hierarchy cells once in `QueryChunk.GetStampRow` and retain managed references in the borrowed `StampRow` | Baseline checkpoint `9c33856` under `<baseline-worktree>/artifacts/remaining-api-baseline-correct`; final candidate under `artifacts/remaining-api-accepted-final2`; exact point/row equivalence covered by `StampRowUsesPreparedHierarchyReferencesAndMatchesPointLookup` | `StampRows` changed from `168.72ns / 1.674μs / 16.779μs` to `138.37ns / 1.403μs / 13.956μs` at `100/1K/10K`: ratios `0.820 / 0.838 / 0.832`, all `0 B`. Per-slot reads no longer traverse `World` and both hierarchy buffers. |
...
| Array row-start reference for `netstandard2.1` | Use `fixed` to get a GC-tracked row-start ref under Mono; retain `MemoryMarshal.GetArrayDataReference` for `net10.0` | `78c439b`; 200K entities, 70×100 ms; Mono 6.12 and CoreCLR 10; [full evidence](netstandard21-array-reference-2026-09-17.md) | Accepted for `netstandard2.1`: Mono repeat measured `fixed` at `112.2 µs`, versus `160.8 µs` for Span and `236.9 µs` for direct indexing. Current shipping-path rerun measured `113.6 ± 0.29 µs` on Mono and `55.27 ± 0.164 µs` on CoreCLR. `net10.0` remains on its existing runtime path. The runner measures only these shipping implementations; candidate measurements remain historical evidence. |
...
| Historical operation-specific stamp writers | Route entity, chunk and archetype stamp writes according to operation scope; use a separate generated read execution with no write state | Implemented through `e0bf551`, `a654b50` and `e6150af`; [focused report](specialized-stamp-writers.md); matched BDN under `artifacts/stamp-writers-large-{before,after}` | Correctness passed (`136/136` ECS and `21/21` generator tests), allocations stayed `0 B`. Large Movement4 was statistically neutral (`−0.32%` at 100K, error-overlapping). The entity/archetype specialization remains; the former row route and chunk writer were later removed. |
| Structural migration and creation storage | Prepare O(1) partial/empty donor candidates, adopt and pool component rows, bulk-fill added values, copy stamp ranges in bulk, and add handle/count creation without entity output storage | Current candidate after `7418fab`; [short structural report](artifacts/structural-optimizations-20260911-short/Delta.ECS.MicroBenchmarks.QueryBatchStructuralOperationsMicroBenchmarks-report-github.md); .NET 10.0.9 Arm64 RyuJIT on Apple M4 Pro, `100K` entities, ShortRun (`3` warmups, `3` measured iterations), tiering/R2R disabled | Correctness passed (`143/143` ECS tests and contract smoke). Delta measured `107.014 us` Add, `93.417 us` Remove, `4.042 us` Destroy and `456.882 us` CreateBatch, all `0 B` managed allocation; Arch reference measured `244.973 us`, `217.583 us`, `475.528 us` and `1.313 ms`. ShortRun intervals are wide, so these are directional results rather than a publication claim. |
| Structural `Where` execution | Generate direct intercepted predicates and execute filtered `Add`/`Remove`/`Destroy` with one chunk-local run pass, reusable target cursors, archetype-level transition plans, bulk free-list writes and source compaction | Candidate change from `7b65383`; focused BDN on .NET 10.0.9 Arm64 RyuJIT, Apple M4 Pro, `100K` entities, `3` warmups, `10–20` iterations, `200 ms`, one launch, `runOncePerIteration`; runtime `143/143`, generator `39/39`, contract smoke passed | `WhereAddRemove` measured `7.324 ms` mean (`7.326 ms` median, `0.053 ms` StdDev, `0 B`); `DirectForEach` `68.263 us`; `WhereForEachRefReadonly` `57.786 us`. The implementation removes the temporary match bitmask/list and avoids per-entity structural dispatch. Stamp writes remain per-row because real workloads are rarely uniform; no uniform-range speedup is claimed. |

## Rejected as performance improvements

| Candidate | Evidence/result | Reason not retained |
| --- | --- | --- |
| Flatten the public two-loop `scope.Chunks` iterator and carry public archetype/plan state by reference | Baseline checkpoint `9c33856` under `<baseline-worktree>/artifacts/remaining-api-baseline-correct`; candidate under `artifacts/remaining-api-candidate`; .NET 10.0.9 Arm64, tiering/R2R disabled, `10` warmups, `10–20 × 300ms`, one launch | The flat `TwoWhile` path improved only at 100 entities (`40.88→37.62ns`) but regressed at 1K (`390.5→408.1ns`) and 10K (`3.823→3.913μs`). The related `ThreeWhile` borrowed-plan form regressed from `39.49/400.2ns/3.822μs` to `39.92/414.2ns/3.912μs`. Public iterator changes were reverted; the flat query snapshot remains internal where it is already required. |
| Remove the point-write stamp writer and duplicate-looking final store | A/B/A `PointSet` at 10K under `<baseline-worktree>/artifacts/pointset-aba-baseline-{a,b}` and `artifacts/pointset-aba-candidate`; .NET 10.0.9 Arm64, tiering/R2R disabled, `10` warmups, `10–20 × 500ms`, one launch | Baselines were `103.702±0.678μs` and `100.862±0.746μs`; candidate was `108.458±2.967μs` (`+4.6%` and `+7.5%`). Despite fewer source-level operations, the generated/JIT path was slower and multimodal. The candidate was reverted. |
| Native dynamic component mask backed by a `uint` word array | Candidate commits `97cf8e5` and `81ac418`, merged by `20efeb1`; [focused report](native-dynamic-component-mask.md); one-second Movement4 comparison under `<external-baseline-root>/deltaecs-native-dynamic-mask/artifacts/native-dynamic-mask-candidate-long` and `artifacts/native-dynamic-mask-baseline-long` | Functional coverage passed `110/110` and IDs above 255 work without changing public signatures. Throughput was mixed at `−3.8% / +1.2% / +0.8% / −6.0%` for `100 / 1K / 10K / 100K` entities; middle-size errors overlap, while separate-process endpoint results have opposite signs. No stable all-size win, so this is retained as the current functional mask representation, not as a measured throughput optimization. Dynamic word scans and per-mask native allocation/finalization remain cold-path costs. |
| World-owned hierarchical component stamp layers | `6c56cb2` baseline versus its temporary candidate; [focused report](hierarchical-component-stamps.md); BDN reports under `artifacts/hierarchical-stamps-before-{100,100000}` and `artifacts/hierarchical-stamps-after-{100,100000}` | Correctness passed (`136/136` ECS tests and `20/20` generator tests), and `Chunk`/`Archetype` stayed free of extra hierarchy fields. However, the write-heavy Movement4 candidate regressed `+2.1%` at 100 entities and `+1.2%` at 100,000 entities with `0 B` allocation. The four-term candidate was not retained; the current implementation uses the documented two-term mapping. |
...
| Per-archetype generated route carried by the execution driver | `502230a` (reverted by `9d352c1`); JIT `artifacts/perf-round-20260829/candidate-in-ctor-baseline-jit.txt` vs `candidate-archetype-route-cache-jit.txt`; BDN `artifacts/perf-round-20260829/candidate-archetype-route-cache-bdn` vs `<external-baseline-root>/deltaecs-perf-baseline-20260829/artifacts` | JIT shrank `888→884 B` (`sbfiz 4→3`, `add/sub 33→32`, `str 20→19`), but Movement4 throughput regressed `+2.5%/+4.7%/−3.0%/+7.7%/+2.5%` at 100/1k/10k/100k/1M entities; no stable win |
| Passing generated plan/chunk state by `in` | `a32f1f6` (reverted by `f005b4e`); JIT `artifacts/perf-round-20260829/candidate-in-plan-byref-jit.txt`; BDN `artifacts/perf-round-20260829/candidate-in-plan-byref-bdn` vs `<external-baseline-root>/deltaecs-perf-baseline-20260829/artifacts` | JIT grew `888→908 B`, `ldr 68→71`, and `ldp/stp 31→30`; Movement4 changed `+17.8%/+2.4%/−13.1%/−4.5%/+2.8%` at 100/1k/10k/100k/1M entities, with no all-size win |
| Duplicate query-route reference in every `ChunkPlan` | `2e8ea3e` (reverted by `82bb25d`); JIT `artifacts/perf-round-20260829/candidate-chunkplan-routes-jit.txt`; BDN `artifacts/perf-round-20260829/candidate-chunkplan-routes-bdn` vs `<external-baseline-root>/deltaecs-perf-baseline-20260829/artifacts` | JIT shrank `888→884 B` (`sbfiz 4→2`, `add/sub 33→32`, `str 20→19`, `ldp/stp 31→30`), but the extra 8-byte reference per chunk produced mixed Movement4 results: `+0.7%/+2.0%/−3.3%/+0.5%/−1.3%` at 100/1k/10k/100k/1M; errors overlap and there is no stable win |
| Inductive typed refs in generated functor loops | `6f73b63` (reverted by `465a51a`); JIT `artifacts/perf-round-20260829/candidate-functor-inductive-refs-jit.txt`; BDN `artifacts/perf-round-20260829/candidate-functor-inductive-refs-bdn` vs `<external-baseline-root>/deltaecs-perf-baseline-20260829/artifacts` | JIT shrank `888→872 B` (`sbfiz 4→3`, `cmp 5→4`), but Movement4 changed `+11.9%/+3.0%/−2.7%/−5.1%/+1.6%` at 100/1k/10k/100k/1M; allocation stayed `0 B` and no all-size throughput win |
| Pass component-row array directly to `GeneratedQuerySlots` | `66be0e2` (reverted below); JIT `artifacts/perf-round-20260829/candidate-slots-array-constructor-jit.txt`; BDN `artifacts/perf-round-20260829/candidate-slots-array-constructor-bdn` vs `<external-baseline-root>/deltaecs-perf-baseline-20260829/artifacts` | JIT stayed `888 B` with the same `blr=5` and hot-loop shape; Movement4 changed `+6.1%/+0.5%/−1.1%/−1.9%/+1.2%` at 100/1k/10k/100k/1M, with larger error on the candidate and no stable win; allocation stayed `0 B` |
...
| Use `RuntimeTypeHandle` keys in primary route/access maps | `ea5cb65` (reverted below); JIT `artifacts/perf-round-20260829/candidate-runtime-type-handle-jit.txt`; BDN `artifacts/perf-round-20260829/candidate-runtime-type-handle-bdn` vs fresh baseline `<external-baseline-root>/deltaecs-perf-baseline2-artifacts` | Preserved all type/query validation and `0 B` allocation. Fresh BDN changed `−10.0%/+2.4%/−5.9%/−5.9%/+3.6%` at 100/1k/10k/100k/1M; errors overlap at the small sizes and 1M regressed, so no stable all-size win |
| Hoist generated query-component route extraction out of the chunk loop | Candidate source in `src/DeltaECS.Generators/DemandDrivenForEachGenerator.cs`; BDN `artifacts/route-locals-two-100` and `artifacts/route-locals-two-100k`; preceding type-handle comparison `artifacts/typehandle-two-100k` | No unrolling and `0 B` allocation. At 100K, Delta changed from `27.36 ±0.126 μs` to `27.191 ±0.238 μs` (about `0.6%` faster, overlapping intervals) while Myriad measured `26.643 ±0.190 μs`; at 100 entities Delta was `33.273 ±0.393 ns` versus Myriad `46.013 ±0.232 ns`. Keep as an inconclusive generated-code simplification until a same-process baseline confirms a stable gain |
...
| Add `SkipLocalsInit` to generated closed execution methods | Candidate patch in `src/DeltaECS.Generators/DemandDrivenForEachGenerator.cs`; Release build output `<external-baseline-root>/deltaecs-skiplocals-build2.log` | Rejected at the compilation-safety gate before JIT/BDN. The generated consumer methods produced `CS0227` because `SkipLocalsInitAttribute` requires unsafe compilation, while the benchmark consumer is intentionally not compiled with `/unsafe`. Enabling unsafe for consumers would change the build contract for a speculative internal optimization, so the patch was reverted and no performance claim was made. |
| Re-test `SkipLocalsInit` with unsafe consumers | Candidate patch in `src/DeltaECS.Generators/DemandDrivenForEachGenerator.cs` plus temporary `<AllowUnsafeBlocks>true</AllowUnsafeBlocks>` in the generated-consumer projects; JIT `artifacts/perf-round-20260829/candidate-skiplocals-jit.txt`; strict BDN `artifacts/perf-round-20260829/candidate-skiplocals-movement4` versus `artifacts/perf-round-20260829/baseline-accurate-rerun` | Rejected. The explicit unsafe permission removed the earlier compilation blocker, but the closed Movement4 method remained `888 B` with `blr=5`, `bounds=1`, `sbfiz=4`, `ldr=68`, `str=20`, and `ldp/stp=31`. Under .NET 10.0.9 Arm64 RyuJIT with tiering/R2R disabled, `IterationTime=1000 ms`, `WarmupCount=10`, `MinIterationCount=10`, `MaxIterationCount=20`, `LaunchCount=3`, candidate/base means and 99.9% errors were `149.589±1.589 ns / 148.871±0.886 ns` (+0.48%), `1.157±0.0039 μs / 1.156±0.0032 μs` (+0.09%), `12.229±0.033 μs / 12.192±0.077 μs` (+0.30%), `120.270±0.72 μs / 120.097±0.59 μs` (+0.14%), and `1.195±0.0079 ms / 1.202±0.0083 ms` (−0.58%) for `100 / 1K / 10K / 100K / 1M` entities. Every interval overlapped; no stable throughput or code-size improvement. The temporary unsafe/build and generator changes were reverted. |
| Split `OpenDense` into read/write-specialized compiler-support entries | Candidate source diff in `src/DeltaECS/Generator/GeneratedRuntime.cs` and `src/DeltaECS.Generators/DemandDrivenForEachGenerator.cs`; JIT `artifacts/perf-round-20260829/candidate-open-specialized-jit-report.md`; screening BDN `artifacts/perf-round-20260829/candidate-open-specialized/amount100` versus corrected baseline `artifacts/perf-round-20260829/baseline-accurate-rerun/amount100` | Rejected. Specializing the internal entry removed the constant `hasWrites` argument setup from the closed Movement4 method (`888→884 B`, one `mov` removed), while `blr=5`, `sbfiz=4`, `ldr=68`, `str=20`, `ldp/stp=31` and branch counts were unchanged. Strict .NET 10.0.9 Arm64 RyuJIT screening (`IterationTime=1000 ms`, `WarmupCount=10`, `MinIterationCount=10`, `MaxIterationCount=20`, `LaunchCount=3`, tiering/R2R disabled) measured `149.3 ±0.67 ns` versus baseline `148.9 ±0.89 ns` at 100 entities (`+0.27%`, overlapping error). The source diff was reverted; no stable throughput benefit. |
| Frozen type-key route/access maps | Candidate `src/DeltaECS/Core/QueryAccess.cs`; JIT `artifacts/perf-round-20260829/candidate-frozen-type-map-jit.md`; strict paired Dense BDN `artifacts/perf-round-20260829/baseline-accurate-rerun` versus `artifacts/perf-round-20260829/candidate-frozen-movement4-correct`; strict paired Sparse BDN `artifacts/perf-round-20260829/baseline-frozen-sparse-strict` versus `artifacts/perf-round-20260829/candidate-frozen-sparse-strict`; construction BDN `artifacts/perf-round-20260829/baseline-frozen-query-construction-strict` versus `artifacts/perf-round-20260829/candidate-frozen-query-construction-strict` | Rejected. Replacing the prepared `Dictionary<Type, ...>` maps with `FrozenDictionary<Type, ...>` kept the closed Movement4 method at `888 B` with the same `blr=5`, `bounds=1`, `sbfiz=4`, `ldr=68`, `str=20`, and `ldp/stp=31`. Dense candidate/base deltas were `−1.96%/−0.52%/−0.66%/−0.37%/−0.83%` at `100/1K/10K/100K/1M`; Sparse deltas were `−3.24%/+1.01%/+0.59%/−0.03%` at `100/1K/10K/100K`. Most 99.9% intervals overlapped, so there is no stable all-size throughput win. Query-plan construction was neutral within error at larger amounts. Source was reverted. |
...
| --- | --- | --- |
| Batched `Create` without entity output | Candidate source is the current worktree after `05e0455`; the benchmark project references `src/DeltaECS` directly. .NET 10.0.9 Arm64 RyuJIT on Apple M4 Pro, `100K` entities, `3` warmups, `5` measured iterations, `100 ms`, one launch. `CreateEntityWithOne/Two/ThreeComponents` compares `DeltaECSBatch` and `DeltaECSBatchGeneric` with `Frent_Bulk`. | The append-only kernel reserves all records once and writes entity rows/records directly; stamp value storage is lazy and the obsolete `ListSpanCompat` layer is removed. Results (`Mean`, managed `Allocated`): one component `206.1 us / 468160 B` (archetype) and `225.5 us / 468200 B` (generic) versus `Frent_Bulk 249.9 us / 3403000 B`; two components `246.5 us / 875840 B` and `237.2 us / 875880 B` versus `314.9 us / 3803088 B`; three components `262.3 us / 1283520 B` and `232.1 us / 1283560 B` versus `328.5 us / 4203176 B`. Delta is `17.5%–29.4%` faster than the fastest external batch baseline in this run, with lower managed allocation. Runtime tests `147/147`, generator tests `44/44`, and Release build passed. Recheck across a second process is still required before moving this entry to Accepted. |
| Generated pointer-walk loop for intercepted callbacks | Candidate source in `src/DeltaECS.Generators/DemandDrivenForEachGenerator.cs`; JIT probe in `/tmp/jit-pointer-one.out`; paired BDN under `artifacts/index-walk-baseline-one-1024-100k`, `artifacts/pointer-walk-one-1024-100k`, `artifacts/index-walk-baseline-three-1024-100k` and `artifacts/pointer-walk-fair-three-1024-100k`; competitor checks under `artifacts/myriad-fair-{one,three}-1024-100k` | No loop unrolling. For intercepted callbacks without an entity parameter, advance each validated typed row reference once per entity and use `while (count > 0)`. At `100K` entities with matched `1024`-slot chunks, Delta changed from `28.52 ±0.148 μs` to `24.88 ±0.332 μs` for one component (`−12.8%`) and from `42.50 ±0.138 μs` to `35.73 ±0.281 μs` for the fair three-component workload (`−15.9%`), with `0 B` allocation. The generated one-component caller's hot loop uses post-indexed pointer advancement and removes the indexed `sbfiz`/address-add sequence. Against Myriad's non-unrolled chunk path (`25.67 ±0.140 μs` and `36.10 ±0.828 μs`), the candidate is `3.1%` and `1.0%` faster respectively; the latter is within overlapping error. With the default Delta `512`-slot chunks, the full one-component comparison was `26.261 ±0.122 μs` versus Myriad chunk `26.090 ±0.193 μs`, so the pointer-walk improvement is not an unconditional competitor win. Entity-bearing intercepted methods keep the indexed loop because they require `EntityAt(index)`. |
| Myriad-aligned chunk/row endpoint | Candidate source in `src/DeltaECS/Core/World.cs` and `src/DeltaECS/Generator/GeneratedQuerySlots.cs`; current one-component confirmation under `artifacts/current-inprocess-one`; two-component control under `artifacts/current-inprocess-two`; strict one-component confirmation under `artifacts/chunk1024-pointer-typed-one-100k` and `artifacts/chunk1024-delta-one-{amount}`; chunk-size screening under `artifacts/current-inprocess-two-2048` and `artifacts/current-inprocess-two-4096` | No loop unrolling. The current Delta chunk capacity is fixed at `512`; the cited `1024`-slot measurements belong to an earlier experimental build used to match Myriad. Trusted generated row access uses a typed-array cast at the first element. Strict external-process one-component measurements were `30.89±0.377 / 264.0±1.53 / 2.526±0.0478 / 24.852±0.193 ns/operation` versus Myriad `44.20±0.202 / 286.4±1.92 / 2.580±0.0119 / 25.351±0.105` at `100/1K/10K/100K` entities: Delta wins `30.1% / 7.8% / 2.1% / 2.0%`; the 10K interval overlaps, the other three do not. The fresh in-process confirmation also wins one component, but is only a smoke-strength check. For the fair two-component one-write/one-read control, strict `100K` was `27.84±0.094 μs` versus `27.47±0.126 μs`; the difference is `+1.35%` for Delta and errors overlap. Chunk-size screening at `100K` with in-process `10×200 ms` gave `27.377±0.228 μs` vs `27.196±0.114 μs` at `2048`, and `26.920±0.199 μs` vs `26.958±0.102 μs` at `4096`; both intervals overlap, so neither is a proven win. This aligns the storage/loop shape with Myriad but does not prove a universal multi-component win. |

The following loop-shape checks were also run against the same fair two-component Myriad chunk path at `100K` entities (`IterationTime=200 ms`, `WarmupCount=10`, `IterationCount=10`, `LaunchCount=1`, in-process, no unrolling):

| Loop-shape check | Evidence | Result |
| --- | --- | --- |
| Decrement in the loop condition (`while (count-- > 0)`) | `artifacts/count-loop-two-100k/results/Ecs.CSharp.Benchmark.SystemWithTwoComponents-report-github.md` | Rejected. Delta `48.33±0.426 μs` versus Myriad `26.88±0.201 μs`; the generated code regressed by about `81.1%`. The source was restored to a separate test-body decrement. |
| `do/while` for non-empty trusted chunks | `artifacts/dowhile-two-100k/results/Ecs.CSharp.Benchmark.SystemWithTwoComponents-report-github.md` | Rejected. Delta `27.83±0.144 μs` versus Myriad `26.73±0.170 μs`; Delta remained about `4.10%` slower, worse than the restored `while` form. |
| Direct typed row locals with post-increment pointer walk | `artifacts/direct-row-two-100k/results/Ecs.CSharp.Benchmark.SystemWithTwoComponents-report-github.md` | Retained as the current uncommitted candidate, but it does not beat Myriad on this write/read workload: Delta `27.43±0.140 μs` versus Myriad `26.71±0.215 μs` (about `2.69%` slower). |
...
| Reusable chunk-disjoint parallel Movement4 execution | Candidate source in `src/DeltaECS/Parallel/`; BDN `artifacts/parallel-iteration-smoke-rerun/results/Delta.ECS.Benchmarks.ParallelMovement4IterationBenchmarks-report-github.md` | Inconclusive as a universal replacement for the generated sequential path. The static-range worker pool is safe and allocation-free after warm-up; the rerun measured parallel/sequential ratios `1.13x/1.06x/0.35x/0.33x/1.87x/0.27x` at `100/1K/10K/100K/1M/5M` entities. The 1M point is invalidated by scheduling outliers (`±185%`), while smaller points pay coordination overhead and 5M shows a stable `3.67x` speedup in this short run. Retain as an isolated API experiment until a longer run establishes a stable threshold and worker wake-up policy. |
