...
| Operation-specific stamp writers | Route entity, chunk, archetype and world stamp writes according to operation scope; use a separate generated read execution with no write state | Current uncommitted experiment from `6c56cb2`; [focused report](specialized-stamp-writers.md); matched BDN under `artifacts/stamp-writers-large-{before,after}` | Correctness passed (`136/136` ECS and `21/21` generator tests), allocations stayed `0 B`. Large Movement4 was statistically neutral (`−0.32%` at 100K, error-overlapping); retained as a state/layout specialization, not a throughput claim |

## Rejected

| Candidate | Evidence/result | Reason not retained |
| --- | --- | --- |
| Native dynamic component mask backed by a `uint` word array | Candidate commits `97cf8e5` and `81ac418`; [focused report](native-dynamic-component-mask.md); one-second Movement4 comparison under `/private/tmp/deltaecs-native-dynamic-mask/artifacts/native-dynamic-mask-candidate-long` and `/Users/rum/GitProjects/TheFurnace/DeltaECS/artifacts/native-dynamic-mask-baseline-long` | Functional coverage passed `110/110` and IDs above 255 work without changing public signatures. Throughput was mixed at `−3.8% / +1.2% / +0.8% / −6.0%` for `100 / 1K / 10K / 100K` entities; middle-size errors overlap, while separate-process endpoint results have opposite signs. No stable all-size win. Dynamic word scans and per-mask native allocation/finalization add cold-path cost; keep isolated and do not merge as a performance replacement. |
| World-owned hierarchical component stamp layers | `6c56cb2` baseline versus the current uncommitted candidate; [focused report](hierarchical-component-stamps.md); BDN reports under `artifacts/hierarchical-stamps-before-{100,100000}` and `artifacts/hierarchical-stamps-after-{100,100000}` | Correctness passed (`136/136` ECS tests and `20/20` generator tests), and `Chunk`/`Archetype` stayed free of extra hierarchy fields. However, the write-heavy Movement4 candidate regressed `+2.1%` at 100 entities and `+1.2%` at 100,000 entities with `0 B` allocation. Keep the centralized storage only if the hierarchy semantics are required; no throughput win is claimed. |
