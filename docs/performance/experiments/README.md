...
| Operation-specific stamp writers | Route entity, chunk and archetype stamp writes according to operation scope; use a separate generated read execution with no write state | Implemented through `e0bf551`, `a654b50` and `e6150af`; [focused report](specialized-stamp-writers.md); matched BDN under `artifacts/stamp-writers-large-{before,after}` | Correctness passed (`136/136` ECS and `21/21` generator tests), allocations stayed `0 B`. Large Movement4 was statistically neutral (`−0.32%` at 100K, error-overlapping); retained as a state/layout specialization, not a throughput claim. The global world stamp was removed by `e6150af`. |
